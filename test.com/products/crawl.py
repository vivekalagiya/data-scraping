...
SITE_CONFIG = {
    ...
    "group": {
        "main_selector": ".product-list-item",
...
class Group:
    ...
    def tables(soup, url):
        ...
        while page_num < 20:
            ...
            page_url = f"{url}{separator}PageNumber={page_num}&PageSize=100"
            if page_url in visited:
                break
            visited.add(page_url)

            html = Core.fetch_html(page_url, "zyte")
            if not html:
                break
...
class Core:
    ...
    def fetch_html_with_zyte(url: str, max_retries: int = 3, selector = None) -> str:
        ...
        payload = {
                    ...
                    "actions": [{
                        ...
                        "selector": {"type": "css", "value": ".product-list-item"},
                        "timeout": 20
                    }]
                }
        if selector:
    ...
    def get_requests(url, headers=None, timeout=60,stream=False, retries=3):
        ...
        try:
            ...
            # )
            # html = api_response.json().get("browserHtml")
    ...
    def fix_lazy_loaded_images(soup):
        for img in soup.find_all("img"):
            ...
            for attr in ["data-amsrc", "data-src", "data-original"]:
...
def init(url, update_prices_only=False):
    ...
    try:
        ...
        logger.info(f"Category selector found: {bool(soup.select_one('.category-list'))}")
        logger.info(f"Group selector found: {bool(soup.select_one('.product-list-item'))}")
        logger.info(f"Product selector found: {bool(soup.select_one('.product-details'))}")
        logger.info(f"Products found: {len(soup.select('.product-list-item'))}")
        logger.info("------------------------------")
        Core.fix_lazy_loaded_images(soup)
    ...
    # return crawl_array
    if not crawl_array:
            logger.error("No page type detected.")
            return None
