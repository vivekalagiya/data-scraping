...
class Product:
    def tables(soup, url):
        ...
        Category_tag = (
            soup.select_one(".products-details .category-name") or
            soup.select_one("ol.breadcrumb li:last-child") or
            soup.select_one(".product-category") or
            soup.select_one("span.cate-name") or
            soup.select_one(".detail-product-category")
        )
            ...
            for panel in panels:
                ...
                # ---------- Features & Benefits (ul → list)
                    ...
                    if lis:
                        ...
                        text = " ".join(body.stripped_strings)
                        if text:
                            features = [t.strip() for t in text.split("·") if t.strip()]
                    if features:
                ...
                # -------Application
                    ...
                    if lis:
                        ...
                        text = " ".join(body.stripped_strings)
                        if text:
                            applications = [t.strip() for t in text.split("·") if t.strip()]
                    if applications:
                        product_data["Application"] = applications
        ...
        # Fallback PDF link
        if not product_data.get("pdf_link"):
            datasheet_link = soup.select_one("a[href*='image_product']")
            if datasheet_link:
                href = datasheet_link.get("href", "").strip()
                if href:
                    product_data["pdf_link"] = href
                    product_data["pdf_filename"] = href.split("/")[-1]

        # -------------------------------------------------------
        #  Order Information table
        # -------------------------------------------------------
        ordering_table = None
        for heading in soup.find_all(["h2", "h3", "h4", "strong", "th"]):
            if "ordering information" in heading.get_text(strip=True).lower():
                next_table = heading.find_next("table")
                if next_table:
                    ordering_table = next_table
                break

        if not ordering_table and accordion:
            for panel in accordion.select(".panel-heading a"):
                section_name = panel.get_text(" ", strip=True).replace("+", "").strip()
                if "ordering" in section_name.lower():
                    target_id = panel.get("href", "")
                    if target_id.startswith("#"):
                        panel_div = accordion.find(id=target_id[1:])
                        if panel_div:
                            ordering_table = panel_div.select_one("table")
                    break

        if ordering_table:
            ordering_info = Product._parse_ordering_table(ordering_table, base_url)
            if ordering_info:
                product_data["Ordering_Information"] = ordering_info
    ...
    # -------------------------------------------------------
    # Helper method to parse ordering table
    # -------------------------------------------------------
    def _parse_ordering_table(table, base_url):
        ...
        result = {}
        for i, cell in enumerate(data_rows[0].select("td")):
            if i >= len(headers):
                break
    ...
    def documentation(soup):
        ...
        for sel in SITE_CONFIG["part"]["documentation"]:
            for a in soup.select(sel):
                ...
                # captue link text as a description
